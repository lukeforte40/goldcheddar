...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+mysqlconnector://root:lforte@127.0.0.1:3306/invsite'
app.permanent_session_lifetime = timedelta(minutes=30)

engine = create_engine(
    'mysql+mysqlconnector://lukeraunak@127.0.0.1:3306/goldcheddardb', pool_recycle=3600
)
Sesh = sessionmaker()
...
@app.route('/moneyManagement/investing/stocks/<id>')
def stockPost(id):
    p = sesh.query(posts).filter(posts.id == id).first()
...
@app.route('/incomeGrowth/passiveIncome/<id>')
def passiveIncPost(id):
    p = sesh.query(posts).filter(posts.id == id).first()
    return render_template('post.html', post = p)
...
@app.route('/incomeGrowth/activeIncome/<id>')
def activeIncPost(id):
    p = sesh.query(posts).filter(posts.id == id).first()
    return render_template('post.html', post = p)
...
@app.route('/incomeGrowth/incgrowth/<id>')
def incGrowthPost(id):
    p = sesh.query(posts).filter(posts.id == id).first()
    return render_template('post.html', post = p)
